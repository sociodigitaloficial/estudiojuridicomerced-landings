...
// ---------------------------------------- BASE DE DATOS
    ...
    $server = "localhost";
    $user = "sociodig_user_estudiojuridicomerce";
    $password ="[zb;e~2v74L%";
    $database = "sociodig_estudiojuridicomerce";
...
// >>> Cliente Interno
	...
	if(!stripos($nombre,'prueba')){
		$mail->addAddress('eduardoarenasamer@gmail.com');
	}
	
	$mail->isHTML(true);
    $mail->Subject = 'Estudio Jurídico Merced has recibido un NUEVO CONTACTO';
	
$message = '<html><body>';
	$message .= "<p>Hola Estudio Jurídico Merced, gracias a la campaña de publicidad digital que actualmente tienes online has recibido un nuevo contacto interesado en tus servicios:</p>";
	...
	$message .= "<tr><td><strong>Landing:</strong> </td><td>" . strip_tags($servicio) . "</td></tr>";
...
// >>> Cliente Externo
	...
	try {
		...
		$mail2->SMTPAuth   = true;
		$mail2->Username   = 'notificaciones@estudiojuridicomerce.cl';
		$mail2->Password   = '(+mA)KR8G4Td';
		$mail2->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;
		$mail2->Port       = 465;   
		$mail2->CharSet = 'UTF-8';
				
		$mail2->setFrom('notificaciones@estudiojuridicomerce.cl', 'Estudio Jurídico Merced');
		$mail2->addAddress($correo);
		$mail2->addReplyTo('eduardoarenasamer@gmail.com');
		...
		$message .= "<p>Hola " . strip_tags($nombre) . ", gracias por confiar en nosotros y solicitar una asesoría en divorcios.</p>";
			
		if(date('N')<=5){
			
			if(date('G')<18){ $message .= "<p>Durante la jornada nos contactaremos contigo para resolver tus consultas de la mejor manera posible y con toda la información pertinente. ¡Hablamos pronto!.</p>";}
